...
import { render, act, fireEvent, waitFor, within, isInaccessible } from '@testing-library/svelte'
...
describe('<VariableFont />', () => {
  ...
  afterEach(() => {
    ...
    it('renders the font name and size', async () => {
      ...
      await waitFor(() => expect(component.getByRole('banner').textContent).toMatch(`${font} (${100}KB)`))
    })
    ...
    it('renders the font options', async () => {
      expect(variationOptions.length).toEqual(Object.entries(Object.values(fonts)[0].options).length)
  ...
  describe('interacting', () => {
    ...
    beforeEach(() => {
      fireEvent.input(textArea, { target: { value: 'the quick brown fox jumped over the lazy dog' } })
    ...
    it('applies the styles', () => {
      const styles = textArea.parentElement.attributes.style.value
